...
#if defined(NDEBUG)
# error "LightningCashr cannot be compiled without assertions."
#endif
...
const int nYesPowerFork = 10; // 247777
const int nSpeedFork = 10; // ???? 310000
...
const std::string strMessageMagic = "Lightning Signed Message:\n";   // LightningCashr: Should still use LTC's strMessageMagic so that pre-fork sigs validate
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s LNCR additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
   ...
   /* if (nHeight == consensusParams.lastScryptBlock + 1)
    ...
    // LightningCashr: Slow-start the first n blocks  blocks to prevent early miners having an unfair advantage
...
// LightningCashr: Hive: Return the current cost for a single worker bee
...
void ThreadScriptCheck() {
    RenameThread("lightningcashr-scriptch");
...
void static UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++)
        {
            ...
            // LightningCashr: Hive: Don't warn about unexpected version in Hivemined blocks
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // LightningCashr: Hive: Check Hive proof
    ...
    // Check transactions
    for (const auto& tx : block.vtx)
        if (!CheckTransaction(*tx, state, true))    // LightningCashr: Fix CVE-2018-17144
...
// LightningCashr: Hive: Check if Hive is activated at given point
...
// LightningCashr: Hive: Check if Hive 1.1 is activated at given point
...
// LightningCashr: Hive: Check if Hive 1.2 is activated at given point
...
// LightningCashr: Hive: Get the well-rooted deterministic random string (see whitepaper section 4.1)
...
// LightningCashr: Hive: Get tx by given hash, from a block at given chain height
...
//	return true;
    ...
    // LightningCashr: Hive: Check appropriate Hive or PoW target
