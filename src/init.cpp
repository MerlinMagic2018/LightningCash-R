...
void Shutdown()
{
    ...
    RenameThread("lightningcashr-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    // LightningCashr: Hive: Mining optimisations
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("lightningcashr-loadblk");
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if lightningcashr is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if lightningcashr is started while in a temporary directory.\n",
    ...
    // LightningCashr: Hive: Start the mining thread
#ifdef ENABLE_WALLET
