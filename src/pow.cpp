...
#include <uint256.h>
#include <util.h>
#include <core_io.h>            // LightningCashr: Hive
#include <script/standard.h>    // LightningCashr: Hive
#include <base58.h>             // LightningCashr: Hive
#include <pubkey.h>             // LightningCashr: Hive
#include <hash.h>               // LightningCashr: Hive
#include <sync.h>               // LightningCashr: Hive
#include <validation.h>         // LightningCashr: Hive
#include <utilstrencodings.h>   // LightningCashr: Hive

BeePopGraphPoint beePopGraph[1024*40];       // LightningCashr: Hive
...
// LightningCashr: DarkGravity V3 (https://github.com/dashpay/dash/blob/master/src/pow.cpp#L82)
...
{
    const arith_uint256 bnPowLimit = UintToArith256(params.powLimit);   // LightningCashr: Note we use the Scrypt pow limit here!
    int64_t nPastBlocks = 24;

    // LightningCashr: Allow minimum difficulty blocks if we haven't seen a block for ostensibly 10 blocks worth of time
    if (params.fPowAllowMinDifficultyBlocks && pblock->GetBlockTime() > pindexLast->GetBlockTime() + params.nPowTargetSpacing * 10)
        return bnPowLimit.GetCompact();

    // LightningCashr: Make sure we have at least (nPastBlocks + 1) blocks since the fork, otherwise just return powLimitSHA
    ...
    for (unsigned int nCountBlocks = 1; nCountBlocks <= nPastBlocks; nCountBlocks++) {
        // LightningCashr: Hive: Skip over Hivemined blocks; we only want to consider PoW blocks
    ...
    // LightningCashr : Limit "High Hash" Attacks... Progressively lower mining difficulty if too high...
...
{
    const arith_uint256 bnPowLimit2 = UintToArith256(params.powLimit2);   // LightningCashr: Note we use the Scrypt pow limit here!
    int64_t nPastBlocks = 24;
    ...
    // LightningCashr: Hive 1.1: Skip over Hivemined blocks at tip
...
unsigned int GetNextWorkRequiredLTC(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // LightningCashr: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if ((pindexLast->nHeight+1 >= nYesPowerFork-5) && (pindexLast->nHeight+1 < nYesPowerFork))
...
unsigned int CalculateNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
    ...
    bnOld = bnNew;
    // LightningCashr: intermediate uint256 can overflow by 1 bit
...
// LightningCashr: Hive 1.2: SMA Hive Difficulty Adjust
...
// LightningCashr: Hive: Get count of all live and gestating BCTs on the network
	...
	//    CAmount beeCost = GetBeeCost(chainActive.Height(), consensusParams);
            ...
            if (block.vtx.size() > 0) {
			    ...
			    if ((chainActive.Tip()->nHeight) >= nContribFork)
                            	expectedDonationAmount = (beeFeePaid + donationAmount) / consensusParams.communityContribFactor2;  // ...check for valid donation amount
			    else
				expectedDonationAmount = (beeFeePaid + donationAmount) / consensusParams.communityContribFactor;  // ...check for valid donation amount
        ...
        if (consensusParams.isTestnet == false) {
            ...
            if ((!(chainActive.Back(pindexPrev))->GetBlockHeader().IsHiveMined(consensusParams))) {  // Don't check Hivemined blocks (no BCTs will be found in them)
				...
				CAmount expectedDonationAmountX;
			    	if ((chainActive.Tip()->nHeight) >= nContribFork)
                            		expectedDonationAmountX = (beeFeePaidX + donationAmountX) / consensusParams.communityContribFactor2;  // ...check for valid donation amount
			    	else
					expectedDonationAmountX = (beeFeePaidX + donationAmountX) / consensusParams.communityContribFactor;  // ...check for valid donation amount
...
// LightningCashr: Hive: Check the hive proof for given block
...
// Check previous block wasn't hivemined
 ...
 /*   // LightningCashr: Hive 1.2: Check that there aren't too many consecutive Hive blocks
    ...
    CScript bctScriptPubKey;
    {
	    ...
	    CAmount expectedDonationAmount;
	    if ((chainActive.Tip()->nHeight) >= nContribFork)
            	expectedDonationAmount = (bctValue + donationAmount) / consensusParams.communityContribFactor2;  // ...check for valid donation amount
	    else
		expectedDonationAmount = (bctValue + donationAmount) / consensusParams.communityContribFactor;  // ...check for valid donation amount
            if (donationAmount != expectedDonationAmount) {
