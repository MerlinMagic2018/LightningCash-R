...
const char * const BITCOIN_CONF_FILENAME = "lightningcashr.conf";
const char * const BITCOIN_PID_FILENAME = "lightningcashrd.pid";
...
#else
    const char* pszModule = "lightningcashr";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LightningCashr";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LightningCashr";
#else
    // Unix
    return pathRet / ".lightningcashr";
#endif
#endif
}
...
// LightningCashr: Hive: Mining Optimisations: Return number of virt cores
