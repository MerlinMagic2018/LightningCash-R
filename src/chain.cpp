...
#include <chain.h>
#include <chainparams.h>    // LightningCashr: Hive
#include <util.h>    // LightningCashr: Hive
#include <rpc/blockchain.h>     // LightningCashr: Hive 1.1
#include <validation.h>         // LightningCashr: Hive 1.1
...
// LightningCashr: Hive: Grant hive-mined blocks bonus work value - they get the work value of
...
arith_uint256 GetBlockProof(const CBlockIndex& block)
{
    ...
    if (block.GetBlockHeader().IsHiveMined(consensusParams)) {
        ...
        // LightningCashr: Hive 1.1: Set bnPreviousTarget from nBits in most recent pow block, not just assuming it's one back. Note this logic is still valid for Hive 1.0 so doesn't need to be gated.
        ...
        bnPreviousTarget.SetCompact(pindexTemp->nBits, &fNegative, &fOverflow); // LightningCashr: Hive 1.1: Set bnPreviousTarget from nBits in most recent pow block, not just assuming it's one back
