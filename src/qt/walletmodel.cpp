...
#include <policy/policy.h>  // LightningCashr: Hive: For GetVirtualTransactionSize

WalletModel::WalletModel(const PlatformStyle *platformStyle, CWallet *_wallet, OptionsModel *_optionsModel, QObject *parent) :
    QObject(parent), wallet(_wallet), optionsModel(_optionsModel), addressTableModel(0),
    transactionTableModel(0),
    recentRequestsTableModel(0),
    hiveTableModel(0),  // LightningCashr: Hive
    ...
    cachedNumBlocks(0)
{
    ...
    hiveTableModel = new HiveTableModel(platformStyle, wallet, this);  // LightningCashr: Hive
...
WalletModel::UnlockContext WalletModel::requestUnlock(bool hiveOnly)
{
    ...
    // LightningCashr: Hive: Support unlock for hive mining only
    ...
    return UnlockContext(this, valid, was_locked && !fWalletUnlockHiveMiningOnly); // LightningCashr: Hive: Support unlock for hive mining only
}
...
void WalletModel::getBCTs(std::vector<CBeeCreationTransactionInfo>& vBeeCreationTransactions, bool includeDeadBees) {
	...
	if ((Params().GetConsensus().variableBeecost) && (((chainActive.Tip()->nHeight) - 1) >= (Params().GetConsensus().variableForkBlock)) && (((chainActive.Tip()->nHeight) - 1) >= (Params().GetConsensus().remvariableForkBlock)) && ((chainActive.Tip()->nHeight) < nSpeedFork)) {
		//LogPrintf("OK \n");
        	vBeeCreationTransactions = wallet->GetBCTs3(includeDeadBees, true, Params().GetConsensus());
	}
	if ((Params().GetConsensus().variableBeecost) && (((chainActive.Tip()->nHeight) - 1) >= (Params().GetConsensus().variableForkBlock)) && (((chainActive.Tip()->nHeight) - 1) < (Params().GetConsensus().remvariableForkBlock)) && ((chainActive.Tip()->nHeight) < nSpeedFork)) {
		//LogPrintf("OK \n");
...
bool WalletModel::createBees(int beeCount, bool communityContrib, QWidget *parent, double beePopIndex) {
    ...
    if ((Params().GetConsensus().variableBeecost) && (((chainActive.Tip()->nHeight) - 1) >= (Params().GetConsensus().variableForkBlock)) && (((chainActive.Tip()->nHeight) - 1) >= (Params().GetConsensus().remvariableForkBlock)) && ((chainActive.Tip()->nHeight) < nSpeedFork)) {
	   // LogPrintf("OK \n");
	    if (!wallet->CreateBeeTransaction3(beeCount, wtxNew, reservekeyChange, reservekeyHoney, honeyAddress, changeAddress, communityContrib, strError, Params().GetConsensus())) {
		QMessageBox::critical(parent, tr("Error"), "Bee creation error: " + QString::fromStdString(strError));
		return false;
	    }
    }
    if ((Params().GetConsensus().variableBeecost) && (((chainActive.Tip()->nHeight) - 1) >= (Params().GetConsensus().variableForkBlock)) && (((chainActive.Tip()->nHeight) - 1) < (Params().GetConsensus().remvariableForkBlock)) && ((chainActive.Tip()->nHeight) < nSpeedFork)) {
	   // LogPrintf("OK \n");
	    if (!wallet->CreateBeeTransaction2(beeCount, wtxNew, reservekeyChange, reservekeyHoney, honeyAddress, changeAddress, communityContrib, strError, Params().GetConsensus())) {
		QMessageBox::critical(parent, tr("Error"), "Bee creation error: " + QString::fromStdString(strError));
		return false;
	    }
    }
