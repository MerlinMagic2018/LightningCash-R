...
UniValue GetNetworkHashPS(int lookup, int height) {
	...
	lookup = IsHive12Enabled(pb->nHeight) ? 1 : pb->nHeight % Params().GetConsensus().DifficultyAdjustmentInterval() + 1;   // LightningCashr: Hive 1.1: Taking the opportunity to provide a more sensible default.
...
// LightningCashr: Hive: Mining optimisations: Set hive mining params
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated lightningcashr to.\n"
...
UniValue getmininginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"errors\": \"...\"            (string) DEPRECATED. Same as warnings. Only shown when lightningcashrd is started with -deprecatedrpc=getmininginfo\n"
            "}\n"
            "\nExamples:\n"
...
UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    GenerateLNCR (request.params[0].get_bool(), numCpus, Params());
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "LightningCashr is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "LightningCashr is downloading blocks...");
...
UniValue estimatefee(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("estimatefee")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "estimatefee is deprecated and will be fully removed in v0.17. "
            "To use estimatefee in v0.16, restart lightningcashrd with -deprecatedrpc=estimatefee.\n"
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "mining",             "sethiveparams",          &sethiveparams,          {"hivecheckdelay", "hivecheckthreads", "hiveearlyout"} },  // LightningCashr: Hive: Mining optimisations: Set hive mining params
    { "mining",             "gethiveparams",          &gethiveparams,          {} },  // LightningCashr: Hive: Mining optimisations: Get hive mining params
};
